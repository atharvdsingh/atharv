...
export default function WorkCard(experience: Experience) {
  return (
    <div className="flex flex-col">
      ...
      <SubHeading className="text-foreground font-semibold" >Technologies used</SubHeading>
      <div className="flex gap-3 flex-wrap ">
        {experience.technologies.slice(0, 6).map((stack, index) => {
          const Icon = stack.icon;
          return (
            <GhostButton href={stack.name} name={stack.name} className="size-6 cursor-pointer " key={index}>
              <Icon />
        ...
        <SubHeading className="text-foreground font-semibold" >Thing's i did  </SubHeading>
        <div>
...
·
                <SubHeading className="my-0" > {descriptions} </SubHeading>
                </div>
            ))
        }
        </div>
